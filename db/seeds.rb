...
def create_jobs_for(restaurant)
  2.times do
    ...
    restaurant.add_owner(User.all.sample)
    restaurant.add_stocker(User.all.sample)
    restaurant.add_cook(User.all.sample)
  end
end
...
def create_orders_for(restaurant)
  ...
  10.times do
    ...
    order = restaurant.orders.create!(user: User.all.sample) 
...
def clone_restaurant(restaurant, cities, count)
  ...
  count.times do |i|
    BuildSeedRestaurants.perform_async(i, name, slug, status, city_id)
  end
end
